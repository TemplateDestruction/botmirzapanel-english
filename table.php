...
try {
    ...
    if (!$table_exists) {
        ...
        $active_help = "0";
        $sublink = "✅ Subscription link is active.";
        $configManual = "❌ Manual config sending is disabled.";
        ...
        $sublink = "✅ Subscription link is active.";
        $active_phone_iran_text = "0";
        $configManual = "❌ Manual configuration sending is disabled.";
...
2- If no announcement about a service outage has been posted in the channel, send a message to support.
3- Do not send services via SMS; you can use email to send messages.
    ";
    $text_dec_fq = " 
 💡 Frequently Asked Questions ⁉️

1️⃣ Is your VPN's IP static? Can I use it for cryptocurrency exchanges?

✅ Due to the country's network conditions and restrictions, our service is not suitable for trading. Only the location remains static.

2️⃣ If I renew my account before it expires, will the remaining days be lost?

✅ No, the remaining days will be added to your renewed subscription. For example, if you renew your 1-month account 5 days before expiration, your total time will be 5 remaining days + 30 renewal days.

3️⃣ What happens if more than the allowed number of devices connect to one account?

✅ In this case, your data will be consumed faster.

4️⃣ What type of VPN do you offer?

✅ Our VPN service is based on V2Ray and supports multiple protocols, ensuring stable and high-speed access even during network disruptions.

5️⃣ Which country is your VPN server located in?

✅ Our VPN server is located in Germany.
...
✅ Contact support with a screenshot of the error message you are receiving.

8️⃣ Is your VPN guaranteed to stay connected at all times?

✅ Due to the unpredictable nature of the country's internet situation, we cannot provide a guarantee. However, we assure you that we will do our best to maintain the best possible service.

9️⃣ Do you offer refunds?

✅ Refunds are possible if the issue is not resolved from our side.

💡 If you did not find the answer to your question, you can contact 'Support.'";
    $text_channel = "   
        ⚠️ Dear user, you are not a member of our channel.
Please join the channel using the button below.
After joining, click the 'Check Membership' button.";
    if (!$table_exists) {
        ...
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_start','Hello, welcome!')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_usertest','🔑 Test Account')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_Purchased_services','🛍 My Services')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_support','☎️ Support')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_help','📚 Tutorial')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_bot_off','❌ The bot is off, please try again in a few minutes.')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_roll','$text_roll')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_fq','❓ Frequently Asked Questions')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_dec_fq','$text_dec_fq')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_account','👨🏻‍💻 User Profile')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_sell','🔐 Purchase Subscription')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_Add_Balance','💰 Add Balance')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_channel','$text_channel')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_Discount','🎁 Gift Code')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_Tariff_list','💰 Subscription Tariffs')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_dec_Tariff_list','Not set')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_Account_op','🎛 Account Settings')");
    }
    else{
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_start', 'Hello, welcome!')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_usertest', '🔑 Test account')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_Purchased_services', '🛍 My services')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_support', '☎️ Support')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_help', '📚 Tutorial')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_bot_off', '❌ The bot is offline, please try again later.')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_roll', '$text_roll')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_fq', '❓ Frequently Asked Questions (FAQ)')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_dec_fq', '$text_dec_fq')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_account', '👨🏻‍💻 User Account Information')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_sell', '🔐 Buy Subscription')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_Add_Balance', '💰 Add Balance')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_channel', '$text_channel')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_Discount', '🎁 Gift Code')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_Tariff_list', '💰 Subscription Pricing')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_dec_Tariff_list', 'Not set')");
        $connect->query("INSERT IGNORE INTO textbot (id_text, text) VALUES ('text_Account_op', '🎛 Account Settings')");
